-- TABLAS PARA CHAT PRIVADO ENTRE CLIENTE Y TRABAJADOR
-- =============================================================================
...
CREATE POLICY "Solo el sistema puede crear chats" 
    ...
    WITH CHECK (true); -- Esto se controla a nivel de aplicación o función

CREATE POLICY "Solo el sistema puede actualizar chats" 
    ON public.chats FOR UPDATE 
    USING (true); -- Esto se controla a nivel de aplicación o función
...
CREATE POLICY "Usuarios pueden marcar mensajes como leídos" 
    ...
    WITH CHECK (
        -- Solo permitir actualizar el campo is_read
        (OLD.content = NEW.content) AND 
        (OLD.file_url = NEW.file_url) AND
        (OLD.file_type = NEW.file_type) AND
        (OLD.created_at = NEW.created_at) AND
        (OLD.sender_id = NEW.sender_id) AND
        (OLD.chat_id = NEW.chat_id) AND
        (OLD.is_system_message = NEW.is_system_message)
    );
...
BEGIN
    ...
    IF v_exists THEN
        ...
        SELECT id INTO v_chat_id FROM public.chats WHERE oferta_id = p_oferta_id;
        RETURN v_chat_id;
    END IF;
    ...
    VALUES (
        ...
        'Chat iniciado. La oferta ha sido aceptada. Pueden coordinar los detalles aquí.',
        true,
        true
    );
